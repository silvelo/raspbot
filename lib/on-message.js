...
let echoFunction = (msg, match, bot) => {
    ...
    return bot.sendMessage(chatID, message, userID)
...
let executeFunction = (msg, match, bot) => {
    ...
    const child = exec(command, (err, stdout, stderr) => {
        ...
        bot.sendMessage(chatID, resp, userID)
    })

    return bot.sendMessage(chatID, `The pid is : ${child.pid}`, userID)
}
...
exports.watchAllFiles = function watchAllFiles(chatID, file, bot) {
    ...
    io.watch(chatID, file, bot)
        ...
        .catch(() => logger.debug('Cannot access to file'))
}
